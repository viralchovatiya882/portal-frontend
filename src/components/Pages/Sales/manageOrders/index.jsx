...
import { cancelSalesOrdersRequest, getManageSalesOrderRequest } from "@store/SalesOrder/sale.actions";
...
/**
 * Renders Manage Sales Order component
 */
const ManageSalesOrder = props => {
  ...
  const fetchSalesOrders = async requestOptions => {
    let salesOrderResp = await props.getManageSalesOrderRequest({ ...requestOptions });

    if (get(salesOrderResp, "error", false)) {
      openNotificationWithIcon("error", "Manage Sales Order", `${get(salesOrderResp, "error.message", "Something Went Wrong")} `);
    }
  ...
  const updateFulfillmentStatus = async (fulfillment_status, sales_order_id, comments) => {
    ...
    if (get(fulFillmentOrderResp, "response.status")) {
      ...
      openNotificationWithIcon(
        "success",
        "Fulfillment Status",
        `${get(fulFillmentOrderResp, "response.message", "Fulfillment Status Successfully")} `
      );
    }
  };
  ...
  return (
    <>
      ...
      <Heading text="Manage Orders" variant="h4" />
        ...
        <ErrorBoundary>
          ...
          <div>
            <CustomTable
              ...
              columnType={TableColumnsList.ManageOrders}
...
export default connect(
  state => ({
    loading: get(state, "salesOrder.manageSalesOrderLoading", false),
    error: get(state, "salesOrder.failure", false),
  }),
  { getCasedGoods, getManageSalesOrderRequest, cancelSalesOrdersRequest }
)(ManageSalesOrder);
